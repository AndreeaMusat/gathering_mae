...
class MapManager:
    ...
    def init_elements(self, map_, init_pos):
        """
        ...
        for ix_map, no_items, x, y, r in init_pos:
            ...
            empty_coord = (map_[x0:x1, y0:y1].sum(0) == DEFAULT_ELEMENT_ID).nonzero()
...
class GatheringEnv(MultiAgentBaseEnv):
                ...
                if len(el_reward_respawn) > 0:
                    while el_reward_respawn[0][0] < step_cnt:
                        ...
                        if init_gen == -1:
                            ...
                            self.el_init.update(map_manager.init_elements(special_map, init_gen))
    ...
    def _reset(self):
        ...
        if self.reward_init_pos:
            self.el_init = self.map_manager.init_elements(self.special_map, self.reward_init_pos)
        else:
            self.el_init = dict()
